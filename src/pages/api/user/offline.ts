...
export default async function userOffline(req: NextApiRequest, res: NextApiResponse) {
  ...
  const response = await User.findOneAndUpdate({ username }, { online: false }, { new: true });
  res
    .status(200)
    .json({
      data: { _id: response._id, username: response.username, online: response.online },
      message: "User updated to offline.",
    });
  res.end();
}
